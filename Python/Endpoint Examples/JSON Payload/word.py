...
if upload_response.ok:
    ...
    word_response = requests.post(url='https://api.pdfrest.com/word',
                        data=json.dumps(word_data),
                        headers={'Content-Type': 'application/json', "API-Key": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"})



    print("Processing response status code: " + str(word_response.status_code))
    if word_response.ok:
        word_response_json = word_response.json()
        print(json.dumps(word_response_json, indent = 2))
