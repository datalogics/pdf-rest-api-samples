 ...
 * - Called from Program.cs to convert PDF to Excel via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- excel /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class Excel
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("excel requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    {
                        excelRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
