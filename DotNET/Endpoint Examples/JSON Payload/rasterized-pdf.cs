 ...
 * - Called from Program.cs to upload a file, then request rasterized-pdf.
 *
 * Setup (environment):
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class RasterizedPdf
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("rasterized-pdf requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    {
                        rasterizeRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
