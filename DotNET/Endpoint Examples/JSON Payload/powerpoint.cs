 ...
 * - Called from Program.cs to convert PDF to PowerPoint via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- powerpoint /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class Powerpoint
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("powerpoint requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    using (var powerpointRequest = new HttpRequestMessage(HttpMethod.Post, "powerpoint"))
                    {
                        ...
                        powerpointRequest.Headers.TryAddWithoutValidation("Content-Type", "application/json");
                        ...
                        powerpointRequest.Content = new StringContent(parameterJson.ToString(), Encoding.UTF8, "application/json"); ;
