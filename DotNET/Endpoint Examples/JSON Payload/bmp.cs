 ...
 * - Called from Program.cs to upload a file, then convert to BMP via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- bmp /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class Bmp
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("bmp requires <inputFile>");
                Environment.Exit(1);
                return;
            }
