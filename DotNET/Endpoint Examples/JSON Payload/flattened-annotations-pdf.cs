 ...
 * - Called from Program.cs to flatten annotations in a PDF via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- flattened-annotations-pdf /path/to/input.pdf
 *
 * Output:
...
{
    public static class FlattenedAnnotationsPdf
    {
        public static async Task Execute(string[] args)
        {
            if (args == null || args.Length < 1) { Console.Error.WriteLine("flattened-annotations-pdf requires <inputFile>"); Environment.Exit(1); return; }
            ...
            using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
            {
                ...
                using (var flattenRequest = new HttpRequestMessage(HttpMethod.Post, "flattened-annotations-pdf"))
                {
                    flattenRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
