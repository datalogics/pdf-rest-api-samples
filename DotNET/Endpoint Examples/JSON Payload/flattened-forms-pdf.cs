 ...
 * - Called from Program.cs to flatten forms in a PDF via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- flattened-forms-pdf /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class FlattenedFormsPdf
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("flattened-forms-pdf requires <inputFile>");
                Environment.Exit(1);
                return;
            }
