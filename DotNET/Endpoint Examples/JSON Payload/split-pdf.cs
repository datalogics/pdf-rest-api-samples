 ...
 * - Called from Program.cs to upload a file, then request split-pdf.
 *
 * Setup (environment):
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class SplitPdf
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("split-pdf requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    {
                        splitRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
