 ...
 * - Called from Program.cs to flatten transparencies in a PDF via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- flattened-transparencies-pdf /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class FlattenedTransparenciesPdf
    {
        ...
        {
            if (args == null || args.Length < 1) { Console.Error.WriteLine("flattened-transparencies-pdf requires <inputFile>"); Environment.Exit(1); return; }
            ...
            using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
            {
                ...
                using (var flattenRequest = new HttpRequestMessage(HttpMethod.Post, "flattened-transparencies-pdf"))
                {
