 ...
 * - Called from Program.cs to unzip an uploaded .zip resource via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- unzip /path/to/files.zip
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class Unzip
    {
        ...
        {
            if (args == null || args.Length < 1) { Console.Error.WriteLine("unzip requires <zipFile>"); Environment.Exit(1); return; }
            ...
            using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
            {
                ...
                {
                    unzipRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
