 ...
 * - Called from Program.cs to convert colors in a PDF to a profile via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- pdf-with-converted-colors /path/to/input.pdf
 *
 * Output:
...
{
    public static class PdfWithConvertedColors
    {
        public static async Task Execute(string[] args)
        {
            if (args == null || args.Length < 1) { Console.Error.WriteLine("pdf-with-converted-colors requires <inputFile>"); Environment.Exit(1); return; }
            ...
            using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
            {
                ...
                {
                    convertColorsRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
                    ...
                    JObject parameterJson = new JObject { ["id"] = uploadedID, ["color_profile"] = "srgb" };
