 ...
 * - Called from Program.cs to compress a PDF via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- compressed-pdf /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class CompressedPdf
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("compressed-pdf requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    {
                        compressRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
