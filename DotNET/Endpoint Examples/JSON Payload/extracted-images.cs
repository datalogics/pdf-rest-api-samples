 ...
 * - Called from Program.cs to upload a file, then request extracted-images.
 *
 * Setup (environment):
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class ExtractedImages
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("extracted-images requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    {
                        extractRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
