 ...
 * - Called from Program.cs to flatten layers in a PDF via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- flattened-layers-pdf /path/to/input.pdf
 *
 * Output:
...
{
    public static class FlattenedLayersPdf
    {
        public static async Task Execute(string[] args)
        {
            if (args == null || args.Length < 1) { Console.Error.WriteLine("flattened-layers-pdf requires <inputFile>"); Environment.Exit(1); return; }
            ...
            using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
            {
                ...
                using (var flattenRequest = new HttpRequestMessage(HttpMethod.Post, "flattened-layers-pdf"))
                {
