 ...
 * - Called from Program.cs to upload a file, then convert to JPG via JSON flow.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- jpg /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.JsonPayload
{
    public static class Jpg
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("jpg requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var httpClient = new HttpClient { BaseAddress = new Uri(baseUrl) })
            {
                using (var uploadRequest = new HttpRequestMessage(HttpMethod.Post, "upload"))
                {
                    ...
                    {
                        jpgRequest.Headers.TryAddWithoutValidation("Api-Key", apiKey);
