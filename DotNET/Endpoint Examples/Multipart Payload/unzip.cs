 ...
 * - Unzips a zip resource via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- unzip-multipart <zipFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- unzip-multipart /path/to/archive.zip
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class Unzip
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("unzip-multipart requires <zipFile>");
                Environment.Exit(1);
                return;
            }
