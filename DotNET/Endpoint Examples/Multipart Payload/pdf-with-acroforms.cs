 ...
 * - Adds acroforms to a PDF via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- pdf-with-acroforms-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- pdf-with-acroforms-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class PdfWithAcroforms
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("pdf-with-acroforms-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
