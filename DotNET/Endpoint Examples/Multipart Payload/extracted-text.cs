 ...
 * - Extracts text via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- extracted-text-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- extracted-text-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class ExtractedText
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("extracted-text-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
