 ...
 * - Converts an input file to PowerPoint via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- powerpoint-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- powerpoint-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class Powerpoint
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("powerpoint-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
