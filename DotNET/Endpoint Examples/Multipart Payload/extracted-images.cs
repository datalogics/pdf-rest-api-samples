 ...
 * - Extracts images via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- extracted-images-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- extracted-images-multipart /path/to/input.pdf
 *
 * Output:
...
{
    public static class ExtractedImages
    {
        public static async Task Execute(string[] args)
        {
            if (args == null || args.Length < 1)
            {
                Console.Error.WriteLine("extracted-images-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
