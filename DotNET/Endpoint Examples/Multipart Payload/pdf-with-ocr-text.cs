 ...
 * - Adds an OCR text layer via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- pdf-with-ocr-text-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- pdf-with-ocr-text-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class PdfWithOcrText
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("pdf-with-ocr-text-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
