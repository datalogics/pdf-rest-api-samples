 ...
 * - Rasterizes a PDF via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- rasterized-pdf-multipart <inputFile>`.
 *
 * Setup (environment):
...
{
    public static class RasterizedPdf
    {
        public static async Task Execute(string[] args)
        {
            if (args == null || args.Length < 1)
            {
                Console.Error.WriteLine("rasterized-pdf-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, "rasterized-pdf"))
            {
