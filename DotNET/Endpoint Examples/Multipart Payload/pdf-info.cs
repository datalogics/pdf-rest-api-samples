 ...
 * - Queries PDF info via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- pdf-info-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- pdf-info-multipart /path/to/input.pdf
 *
 * Output:
 * - Prints the JSON response with requested info. Validation errors exit non-zero.
 */
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class PdfInfo
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("pdf-info-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
