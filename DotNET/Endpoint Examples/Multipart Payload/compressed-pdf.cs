 ...
 * - Compresses a PDF via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- compressed-pdf-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- compressed-pdf-multipart /path/to/input.pdf
 *
 * Output:
...
{
    public static class CompressedPdf
    {
        public static async Task Execute(string[] args)
        {
            if (args == null || args.Length < 1)
            {
                Console.Error.WriteLine("compressed-pdf-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
