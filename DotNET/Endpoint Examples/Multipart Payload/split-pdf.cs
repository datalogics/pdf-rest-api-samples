 ...
 * - Splits a PDF via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- split-pdf-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- split-pdf-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class SplitPdf
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("split-pdf-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
