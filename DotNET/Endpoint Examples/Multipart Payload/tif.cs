 ...
 * - Converts an input file to TIFF using multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- tif-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- tif-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class Tif
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("tif-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
