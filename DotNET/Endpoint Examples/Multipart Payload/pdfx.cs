 ...
 * - Converts a file to PDF/X via multipart/form-data.
 * - Routed from Program.cs as: `dotnet run -- pdfx-multipart <inputFile>`.
 *
 * Setup (environment):
 ...
 *
 * Usage:
 *   dotnet run -- pdfx-multipart /path/to/input.pdf
 *
 * Output:
...
namespace Samples.EndpointExamples.MultipartPayload
{
    public static class Pdfx
    {
        public static async Task Execute(string[] args)
        {
            ...
            {
                Console.Error.WriteLine("pdfx-multipart requires <inputFile>");
                Environment.Exit(1);
                return;
            }
