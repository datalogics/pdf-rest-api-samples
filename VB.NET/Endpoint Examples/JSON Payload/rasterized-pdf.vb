    ...
    Module RasterizedPdf
        Public Async Function Execute(args As String()) As Task
            If args Is Nothing OrElse args.Length < 1 Then
                Console.Error.WriteLine("Usage: dotnet run -- rasterized-pdf /path/to/input.pdf")
                Environment.Exit(1)
            End If
            ...
            Using httpClient As New HttpClient()
                ...
                ' 2) Rasterized PDF request (JSON)
                Dim rasterReq As New HttpRequestMessage(HttpMethod.Post, "rasterized-pdf")
                rasterReq.Headers.TryAddWithoutValidation("Api-Key", apiKey)
                rasterReq.Headers.Accept.Add(New MediaTypeWithQualityHeaderValue("application/json"))

                Dim payload As New Dictionary(Of String, String) From {{"id", uploadedId}}
                Dim payloadJson As String = JsonSerializer.Serialize(payload)
                rasterReq.Content = New StringContent(payloadJson, Encoding.UTF8, "application/json")

                Dim rasterResp As HttpResponseMessage = Await httpClient.SendAsync(rasterReq)
                Dim rasterBody As String = Await rasterResp.Content.ReadAsStringAsync()

                If Not rasterResp.IsSuccessStatusCode Then
                    Console.Error.WriteLine($"Rasterized-pdf request failed: {CInt(rasterResp.StatusCode)} {rasterResp.ReasonPhrase}")
                    Console.Error.WriteLine(rasterBody)
                    Environment.Exit(1)
                End If

                Console.WriteLine(rasterBody)
            End Using
        End Function
    End Module
End Namespace
