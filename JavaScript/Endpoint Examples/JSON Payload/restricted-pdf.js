...
axios(upload_config)
  .then(function (upload_response) {
    ...
    var restrict_config = {
      ...
      data: {
        id: uploaded_id,
        restrictions: ["print_low", "edit_content"],
        new_permissions_password: "Password",
      }, // set the data to be sent with the request
    };

    // send request and handle response or error
    axios(restrict_config)
      .then(function (restrict_response) {
        console.log(JSON.stringify(restrict_response.data));
        var restricted_output_id = restrict_response.data.outputId;
        ...
        var delete_config = {
          ...
          data: { ids: `${uploaded_id}, ${restricted_output_id}` },
        };
