...
$options = [
  'multipart' => [
    ...
    [
      ...
      'contents' => 'pdfrest_word' // Set the value for the output option (in this case, 'pdfrest_word').
    ]
  ]
];

$request = new Request('POST', 'https://api.pdfrest.com/word', $headers); // Create a new HTTP POST request with the API endpoint and headers.

$res = $client->sendAsync($request, $options)->wait(); // Send the asynchronous request and wait for the response.

echo $res->getBody(); // Output the response body, which contains the Word document.
