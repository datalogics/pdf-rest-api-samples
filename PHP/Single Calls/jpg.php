...
$options = [
  'multipart' => [
    ...
    [
      'name' => 'jpeg_quality', // Specify the field name for the jpeg_quality option.
      'contents' => '75' // Set the value for the jpeg_quality option (in this case, '75').
    ],
    [
      'name' => 'output', // Specify the field name for the output option.
      'contents' => 'pdfrest_jpg' // Set the value for the output option (in this case, 'pdfrest_jpg').
    ]
  ]
];

$request = new Request('POST', 'https://api.pdfrest.com/jpg', $headers); // Create a new HTTP POST request with the API endpoint and headers.

$res = $client->sendAsync($request, $options)->wait(); // Send the asynchronous request and wait for the response.

echo $res->getBody(); // Output the response body, which contains the converted JPG image.
