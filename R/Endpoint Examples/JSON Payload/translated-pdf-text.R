#!
# Translate a PDF using two-step JSON flow: upload then /translated-pdf-text.
...
if (is.null(pdf_path) || !file.exists(pdf_path)) {
  stderrf("Usage: Rscript translated-pdf-text.R /path/to/file.pdf\n")
  quit(status = 1)
}
...
tryCatch({
  ...
  url <- paste0(api_base, "/translated-pdf-text")
  ...
  body <- jsonlite::toJSON(list(id = uploaded_id, output_language = "en-US"), auto_unbox = TRUE)
  resp <- httr::POST(
    url,
  ...
  cat(txt)
  if (httr::http_error(resp)) stop(sprintf("Translate failed with status %s", httr::status_code(resp)))
}, error = function(e) {
